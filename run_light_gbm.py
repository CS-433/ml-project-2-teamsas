...
def run_light_gbm(type, datapath_features, datapath_targets, target, features, datapath_features2 = None):
    ...
    if type == 'my_personality':
        ...
        Regression_LGB(X = X_my_personality, y = y_my_personality, type = 'my_personality',  target = target, features = features)
        ...
        Regression_LGB(X = X_idiap, y = y_idiap, type = 'idiap',  target = target, features = features)
        ...
        Regression_LGB(X = X_idiap, y = y_idiap, type = 'idiap_chunked',  target = target, features = features)
