...
from src.transformations import (
    my_personality_target_normalization,
    idiap_target_normalization,
)
...
def get_inputs_chunked_data(
    datapath_features, datapath_targets, features, datapath_features2=None
):

    if features == "psycological":
        data = pd.read_csv(datapath_features)
        X_train = data.iloc[:, 1:]
        X_train_cleaned = X_train.drop(
            columns=[
                "chunk_id",
                "participant_id",
                "contrast",
        ...
        X_train_cleaned = pd.get_dummies(
            X_train_cleaned,
            columns=[
        ...
        X_train_cleaned.columns = X_train_cleaned.columns.str.replace(
